...
Contributions to Gossup are very welcome, but we ask that you read this document
before submitting a PR.

:::note

This document applies to the [Gossup][] repository.
...
- **Backwards compatibility** - Will your change break existing Gossup files? It
...
- **Go** - Gossup is written in [Go][]. We always support the latest two major
  Go versions, so make sure your version is recent enough.
- **Node.js** - [Node.js][] is used to host Gossup's documentation server and is
...
To run Gossup with working changes, you can use `go run main.go overlay`.

### Updating documentation

Gossup uses [Docusaurus][] to host a documentation server. The code for this is
located in the Gossup repository. This can be setup and run locally by using
...
future. Existing tests may also need updating if you have changed Gossup's
behavior.
...
[Gossup]: https://github.com/retr0h/gossup
[Go]: https://go.dev
